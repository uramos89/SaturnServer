...
const head = original.slice(0, 1122).join('\n');
  ...
  const DashboardView = () => (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard label={t('stats.totalServers')} value={servers.length} icon={Server} color="text-blue-500" />
        <StatCard label={t('stats.online')} value={servers.filter(s => s.status === 'online').length} icon={CheckCircle2} color="text-emerald-500" />
        <StatCard label={t('stats.incidents')} value={incidents.filter(i => i.status === 'open').length} icon={AlertTriangle} color="text-rose-500" />
        <StatCard label="SSH Connected" value={sshConnections.filter(c => c.status === 'connected').length} icon={Zap} color="text-orange-500" />
      </div>
          ...
          <section className="space-y-4">
            ...
            <div className="space-y-3">
              {incidents.filter(i => i.status === 'open').length > 0 ? (
                incidents.filter(i => i.status === 'open').slice(0, 5).map(inc => <IncidentCard key={inc.id} incident={inc} t={t} />)
...
fs.writeFileSync(filePath, head + newBody);
